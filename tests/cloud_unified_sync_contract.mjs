...
assert.ok(!src.includes('createTournament('),'unified UI must delegate identity-safe creation to existing canonical Cloud identity layer');
assert.ok(!src.includes("tournament.cloud.syncStatus='LOCAL_CHANGES'"),'result refresh must not invent fake divergence');
