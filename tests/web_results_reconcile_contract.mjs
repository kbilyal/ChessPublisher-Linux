...
const remote=[
 {board:1,whiteKey:'A',blackKey:'B',result:'1-0'},
 {board:2,whiteKey:'C',blackKey:'D',result:'0-1'},
 {board:3,whiteKey:'E',blackKey:'F',result:'1/2-1/2'},
 {board:4,whiteKey:'H',blackKey:'G',result:'1-0'},
 {board:5,whiteKey:'I',blackKey:'J',result:'-'},
];
...
assert(plan.unmatched.length===1,'reversed colors must not be guessed');
assert(plan.remoteBlank===1,'blank web result ignored');
assert(plan.fills[0].localIndex===0&&plan.fills[0].remoteResult==='1 - 0','fill data');
assert(plan.conflicts[0].localIndex===1&&plan.conflicts[0].localResult==='1 - 0'&&plan.conflicts[0].remoteResult==='0 - 1','conflict data');
